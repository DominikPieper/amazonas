...
import { HlmTableComponent, HlmTableModule } from './index';
...
import { BrnToggleGroupModule } from '@spartan-ng/ui-toggle-brain';
  ...
  template: `
    ...
    >
      <span class="text-sm tabular-nums">
        Showing entries {{ ctx.state().startIndex + 1 }} -
        {{ ctx.state().endIndex + 1 }} of {{ _totalElements() }}
      </span>
      <div class="flex">
        <select
          [ngModel]="_pageSize()"
        ...
        >
          <option [value]="size" *ngFor="let size of _availablePageSizes">
            {{ size === 10000 ? 'All' : size }}
          </option>
        </select>
    ...
    <button size="sm" variant="outline" hlmBtn (click)="_loadNewUsers()">
      Mix it up
    </button>
  `,
})
class TableStory {
  ...
  protected readonly _data = computed(() =>
    this._filteredUsers().slice(
      this._startEndIndex().start,
      this._startEndIndex().end + 1,
    ),
  );
  protected readonly _trackBy: TrackByFunction<{ name: string }> = (
    index: number,
    user: { name: string },
  ) => user.name;
...
class TableToggleStory {
  ...
  private readonly _users = signal(createUsers(20));
  private readonly _filteredUsers = computed(() => {
    if (this._onlyAbove180())
      return this._users().filter((u) => u.height > 180);
    return this._users();
  });
  protected readonly _data = computed(() =>
    this._filteredUsers().slice(
