...
} from '@spartan-ng/ui-toggle-brain';
import { HlmIconComponent, provideIcons } from '@spartan-ng/ui-icon-helm';
import { hlmP } from '@spartan-ng/ui-typography-helm';
import { HlmToggleGroupModule } from './index';
  ...
  template: `
    <div class="flex space-x-4">
      ...
      >
        <button
          variant="outline"
          *ngFor="let city of cities; let last = last"
          [value]="city"
          hlm
          brnToggle
        >
    ...
    <p class="${hlmP}">
      {{ multiple ? 'Cities' : 'City' }} selected: {{ selectedCities }}
    </p>
  `,
})
